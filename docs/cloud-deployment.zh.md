...
云端模式的数据工具中提供“云端数据迁移”入口。不要直接覆盖正在运行的 `orders.db`，也不要把单独的 `orders.db-wal` 当作备份使用。

迁移工作台会先检查 SQLite 完整性、结构版本、文件哈希和订单数量，再提供“完整替换”或“安全合并”。执行时会锁定写操作、自动备份当前云端库、校验预览后的云端状态，并在失败时自动恢复。详细规则和正式切换步骤见：

```text
docs/cloud-data-migration.zh.md
```
