...
如果 Jeff 已经使用过绿色版，建议直接发送 `0.1.10` 或更高版本安装包。安装版第一次启动时，如果安装目录里还没有订单数据库，会自动在桌面、下载、文档等常见位置查找旧绿色版的 `data/orders.db`，找到后复制整份旧 `data` 文件夹到安装目录。
...
2. 双击安装包完成安装。
3. 从桌面“Jeff订单工具”图标打开。
4. 如果看到“已自动导入旧版绿色版的数据”，后续只用桌面图标打开，不再打开旧绿色版。
