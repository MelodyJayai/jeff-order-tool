...
V1 聚焦 Jeff 从 2026-06-02 到 2026-08-03 的实际使用反馈：订单号登记、同公司重复使用订单号、自动登记日期、输入订单号查询、出货核销自动日期、按细分类登记数量、客户先交要求、可多次实际先交、剩余未交数量、紧凑订单详情、改单编号、急单提示、电脑和手机浏览器访问。
...
`0.1.28` 起，同一公司可以多次登记相同订单号。每次登记都会生成独立记录，分别保留日期、数量、先交、实际交货和核销状态；订单号本身不会自动增加后缀。搜索结果会显示“同号 N 笔”并同时列出登记日期、工厂和数量，修改与核销始终按内部记录ID执行。

`0.1.29` 起，首页统一显示“未完成订单剩余明细”，并把纯待核销与已先交订单分开计数；订单总表明确标注为“原登记数量”。旧版交货记录如果只记了总件数、没有注明具体品类，工具会明确提示待分品类数量，不再让分品类余额和总剩余看起来像同步错误。

数据库一旦已经产生同公司重复订单号，不要再降级安装 `0.1.27` 或更旧版本；如需回退，应同时恢复升级前自动生成的数据库备份。
...
- 导入 CSV：新版导出文件包含“记录ID”，导入时按记录ID准确更新；旧 CSV 仅在“公司 + 订单号”匹配唯一时兼容更新，存在多笔同号时安全跳过，避免串单。导入前会自动生成一份备份。
- 导入旧版 .db 备份：优先按稳定内部ID合并；没有可用ID的旧数据仅在“公司 + 订单号”匹配唯一时兼容。重复单号会分别保留，交货记录继续归属原订单。导入前会自动生成一份备份。
...
当前建议给 Jeff 发送 `release-installers/JeffOrderToolSetup-v0.1.29.exe`。该版本在稳定内部ID和同号订单支持的基础上，进一步统一未完成订单、剩余未交和原登记数量的界面口径，并明确标记旧记录中未注明品类的已交数量。
...
- 订单使用稳定且不可变的内部 ID；业务订单号只是可搜索编号，同一公司和不同公司都允许重复使用。
...
云端模式提供受登录保护的 `/migration` 数据迁移工作台。上传软件生成的一致性 `.db` 备份后，系统会先检查完整性、结构版本、SHA-256、订单和交货记录数量，再提供完整替换或按稳定内部ID安全合并。旧数据库仅在“公司 + 订单号”一对一且无歧义时兼容对应；同号多笔但ID无法对应时会停止合并。迁移前自动备份，冲突订单必须逐项选择，失败自动恢复，最近一次迁移也可手动回滚。
