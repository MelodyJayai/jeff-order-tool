...
累计先交和剩余未交各用一条横排显示。订单登记后客户临时提出先交要求时，点击醒目的橙色“新增先交数量”即可补录；填写表单默认收起，因此常见笔记本屏幕滚到订单详情顶部后可以在一屏内查看保存、先交记录和核销操作。
...
当前建议给 Jeff 发送 `release-installers/JeffOrderToolSetup-v0.1.21.exe`。该版本包含醒目的追加先交入口、多次先交、剩余未交、紧凑订单详情和旧版 `.db` 导入能力，并使用修复后的 Windows 更新器。
