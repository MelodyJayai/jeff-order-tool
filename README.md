...
累计先交和剩余未交各用一条横排显示。订单登记后客户临时提出先交要求时，在“先交登记”顶部点击醒目的橙色“更改登记：补填先交”即可补录；填写表单默认收起，保存后也会自动收起。
...
当前建议给 Jeff 发送 `release-installers/JeffOrderToolSetup-v0.1.22.exe`。该版本提供明确的“保存登记更改”和“更改登记：补填先交”入口，并修复 Windows 更新助手停止旧服务时误结束自身的问题。由于旧版更新助手本身存在该缺陷，从 `0.1.21` 或更早版本升级到本版时需要手动覆盖安装一次；安装本版后，后续自动更新可正常完成。
